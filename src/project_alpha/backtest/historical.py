...
def simulate_ticker(ticker: str, prices: pd.DataFrame, cost_bps: float = 10.0) -> list[Trade]:
    ...
    for dt, row in series.iloc[WARMUP_ROWS:].iterrows():
        ...
        if position is None:
            ...
            risk = risk_score(annualized_volatility=None if pd.isna(vol) else float(vol))
            tech_risk_score = (tech * _TECH_WEIGHT + risk * _RISK_WEIGHT) / (_TECH_WEIGHT + _RISK_WEIGHT)
            zone = compute_price_zone(price, features)
            in_zone = zone.buy_zone_low * 0.98 <= price <= zone.buy_zone_high
            if tech_risk_score >= TECH_RISK_BUY_THRESHOLD and in_zone:
                position = {
                    "entry_date": dt,
...
def run_universe_backtest(
    ...
    for ticker, prices in price_data.items():
        ticker_trades = simulate_ticker(ticker, prices, cost_bps=cost_bps)
    ...
    return UniverseBacktestResult(all_trades, equity_curve, metrics, counts)
